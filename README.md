...
const Fields = (props: KeyWithAnyModel) => {
  ...
  console.log("isAgevalid",isAgeValidSelector)
  ...
  console.log("userInput",userInputSelector)
  ...
  console.log("mainuserinputs",userInputs)
  ...
  const navigateToNextStage = () => {
     ...
     || stageSelector[0].stageInfo.products[0].product_category === 'PL') && applicationJourney === 'NTC') &&
      ...
      myinfoMissingLogicalFieldsSelector
        ...
        let stateInfofield = stageSelector[0].stageInfo;
                submitBasicDataMyInfo(stateInfofield,channelReferenceBasicData,dispatch).then((response:any)=>{
          if(response){
            dispatch(dispatchLoader(true));
            setBasicResponse(response)
            if (response && response.applicant_documents[0].journey_type) {
              dispatch(dispatchLoader(true));
              dispatch(
                stagesAction.setJourneyType(response.applicant_documents[0].journey_type)
              );
            }
            if(response.applicant_documents[0].journey_type === 'NTC'){
              dispatch(dispatchLoader(true));
              dispatch(stagesAction.updateLastStageInput(stageSelector[0].stageId));
              setFields(
                stageFields(
      ...
      let stateInfofield = {...fieldUpdate.stageInfo,td_multi_currency_products:td_multi_currency_products};
    ...
    else if (stageSelector[0].stageId === "bd-2" && stage.stages.journeyType === "NTC") {
      let channelReferenceBasicData = stageSelector[0].stageInfo.application.channel_reference;
      ...
      .then((response:any) =>{
        ...
        if(response){
          channelReference(payload,channelReferenceBasicData,dispatch).then((res:any)=>{
              if(res){
    ...
    else if (stageSelector[0].stageId === "bd-3" && stage.stages.journeyType === "NTC") {
      let channelReferenceBasicData = stageSelector[0].stageInfo.application.channel_reference;
      const fieldUpdate = JSON.parse(JSON.stringify(basicResponse));
      for (let key in applicantsSelector) {
        fieldUpdate.applicants[key] = applicantsSelector[key];
          ...
          let channelReferenceBasicData = stageSelector[0].stageInfo.application.channel_reference;
            ...
            channelReference(stateInfofield,channelReferenceBasicData,dispatch).then((response)=>{
              ...
              dispatch(
               ...
               dispatch(lovRequests(response?.data, "ad-2"));
                dispatch(stagesAction.updateLastStageInput(stageSelector[0].stageId));
                setFields(
                  stageFields(
                    stageSelector,
     ...
     else if (stageSelector[0].stageId ==="rp") 
    {
     ...
     thankYouPage(fieldUpdate,channelReference,dispatch)
     .then((response) =>{
      if(response){
       dispatch(stagesAction.updateLastStageInput(stageSelector[0].stageId));
       setFields(
    ...
    else if ((stageSelector[0].stageId === "ad-2" && stage.stages.journeyType === "NTC") || 
    (stageSelector[0].stageId === "bd-2" && stage.stages.journeyType === "ETC")) {
      let channelReferenceBasicData = stageSelector[0].stageInfo.application.channel_reference;
      const fieldUpdate = JSON.parse(JSON.stringify(stageSelector[0].stageInfo));
      for (let key in applicantsSelector) {
        fieldUpdate.applicants[key] = applicantsSelector[key];
            ...
            if (response.status === 200) {
              ...
              if (
                ...
                if (stageTo !== "doc" && stageTo !== "rp") {
                  dispatch(lovRequests(response.data, stageTo));
                }
                const stageSelector=[{stageId:stageTo,stageInfo:response.data}];
                setFields(stageFields(stageSelector, stageTo));
                stateUrl("ACD");
  ...
  const validateForm = (e: FormEvent<HTMLFormElement> | null) => {
    console.log("event",e,e?.currentTarget.checkValidity())
    if (e && e.currentTarget.checkValidity() && stageSelector[0].stageId !=="ssf-1") {
      
      setIsFormValid(true);
    }
    else if(e && e.currentTarget.checkValidity() && stageSelector[0].stageId ==="ssf-1"
    &&isAgeValidSelector
    ){
      debugger
      setIsFormValid(true);
  ...
  // const validateDateBasedOnAge = (props:any)=>{
  //   if (props) {
    ...
    <>
      ...
      {stageId !== "doc" && (
        <form className="form" onSubmit={handleSubmit} onChange={validateForm}>
          ...
          {otherMyinfo &&
            isCASAProduct === false && stageId !== "ld-1" && stageId !== "bd-2" && stageId !== "rp" && stageId !=="ad-2" && stageId !=="ACD" &&
